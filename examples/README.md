...
- `basics/02-styled.uxml` — 밝은 초록 카드에 파란 왼쪽 테두리, 밝은 제목과 회색 본문이 보이면 `<Style src>`가 적용된 것이다. 컨트롤 바에서 `hover`를 켰을 때 카드가 짙은 파랑으로 바뀌어야 한다.
...
- `basics/09-states.uxml` — 상태를 하나씩 켰을 때 같은 이름의 행만 각각 분홍(`hover`)·주황(`active`)·청록(`focus`)·보라(`disabled`)로 바뀌면 맞다.
